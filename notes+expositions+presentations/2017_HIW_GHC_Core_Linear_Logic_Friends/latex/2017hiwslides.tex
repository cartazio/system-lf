...
\begin{minted}{haskell}
doit :: Int -> FIXME IO ()
doit ct FIXME = doit . (+) ct =<< atomically foo where
    foo = do
        ma <- tryTake oq1
        mb <- tryTake oq2
        -- FIXME: why not catMaybes [ma, mb] ?
        case (ma, mb) of
            (Nothing, Nothing) -> retry
            (Just ra, Nothing) -> 1 <$ if ct > 10
                then putQ oqB ra else putQ oqA ra
            (Nothing, Just ra) -> 1 <$ if ct > 10
                then putQ oqB ra else putQ oqA ra
            (Just ra, Just rb) -> 2 <$ case compare ct 9 of
                -- FIXME: these don't look right, or at least I'm not seeing the symmetry
                LT -> putQ oqA ra >> putQ oqA rb
                EQ -> putQ oqA ra >> putQ oqB rb
                GT -> putQ oqA rb >> putQ oqA rb
\end{minted}
\end{frame}
...
% EXPLAIN THE SHARED closures conditions
